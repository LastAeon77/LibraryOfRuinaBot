...
"""
    ...
    if icon_id == "":
        image_full_art_path = f"{PRIME_LINK}/django_static/Limbus_Data/skill_art/{skill_id}.png"
    else:
        image_full_art_path = f"{PRIME_LINK}/django_static/Limbus_Data/skill_art/{icon_id}.png"
    return [name,description,image_full_art_path,discord.Color.red()]
...
class ButtonEmbedView(discord.ui.View):
    def __init__(self, author,embeds_list, message=None):
        ...
        for name,description,image_path,color in self.embeds_list:
            button = OneButton(name=name,description=description,image_path=image_path,color=color,image_file=image_path)
...
async def identity_data_analysis(interaction : discord.Interaction,data_in_json,uptie_level,max_level = 45):
    skill_effect = {}
    skill_data = {}
    passives = {}
    ...
    json_passives_battle = sorted([passive for passive in json_passives if not passive['is_support']],key=lambda x:x['uptie_level'])[::-1]
    json_passives_support = sorted([passive for passive in json_passives if passive['is_support']],key=lambda x:x['uptie_level'])[::-1]
...
"""
   ...
   ##################### Skill Data ###############
    ...
    hp = data_in_json["HP"]["defaultStat"] + max_level*data_in_json["HP"]["incrementByLevel"]
    minspeed = sorted(data_in_json["minSpeedList"])[uptie_level-1]
    maxspeed = sorted(data_in_json["maxSpeedList"])[uptie_level-1]
    uniqueAttribute = SIN_DICT.get(data_in_json["uniqueAttribute"],data_in_json["uniqueAttribute"])
...
"""
    ...
    if uptie_level>=3:
        ...
        image_full_art_path = f"{PRIME_LINK}/django_static/Limbus_Data/full_art/{identity_id}_normal.png"
    ...
    for keys in skill_data.keys():
        embed_list.append(skill_description(skill_data=skill_data[keys],
...
async def ego_data_analysis(interaction : discord.Interaction,data_in_json,uptie_level):
    awakeningSkill = None
    corrosionSkill = None
...
"""
    ...
    embed_list.append(skill_description(skill_data=awakeningSkill,
                                            skill_effect=awakeningSkillEffect,
                                            skill_num=1,
                                            ))
    embed_list.append(skill_description(skill_data=corrosionSkill,
