...
from utils.cardScrape import (
    get_site_content_json, get_site_request_logged_in
)
...
class Limbus(commands.Cog):
    def __init__(self, bot):
        ...
        self.identities_list = [(x["title"].replace("\n"," ") + x["name"]) for x in self.iden_dict]
        self.identities_num = [(int(x["id"]), (x["title"].replace("\n"," ") + " " + x["name"])) for x in self.iden_dict]
        ...
        def until_comma(string):
            ...
            return ''.join(final_str)
        self.ego_list = [(x["name"].replace("\n"," ") + " " + until_comma(x["desc"])) for x in ego_dict]
        self.ego_num = [(int(x["id"]), (x["name"].replace("\n"," ") + " " + until_comma(x["desc"]))) for x in ego_dict]
    ...
    async def identity_autocomplete(
        ...
        return [
            ...
            for iden in identities if current.lower() in iden[1].lower()
            ]
    async def ego_autocomplete(
        ...
        return [
            ...
            for ego in egos if current.lower() in ego[1].lower()
            ]

    @app_commands.command()
    @app_commands.autocomplete(identities=identity_autocomplete)
    async def identity(self,interaction: discord.Interaction,identities:str,uptie_level:int =4):
        """Test only"""
        data = await get_site_request_logged_in(f"https://malcute.aeonmoon.page/api/limbus2/identity_data/{identities}")
        await identity_data_analysis(interaction,data,uptie_level,45)
        

    @app_commands.command()
    @app_commands.autocomplete(ego=ego_autocomplete)
    async def ego_limbus(self,interaction: discord.Interaction,ego:str,uptie_level:int =4):
        """Test only"""
        data = await get_site_request_logged_in(f"https://malcute.aeonmoon.page/api/limbus2/ego_data/{ego}")
